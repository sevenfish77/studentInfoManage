...
<script type="text/javascript">
	...
	function searchGrade(){
		$('#dg').datagrid('load',{	//调用load方法
			gradeName:$('#s_gradeName').val()	//返回gradeName的值
		});
	}
	...
	function deleteGrade(){
		...
		for(var i=0;i<selectedRows.length;i++){
			strIds.push(selectedRows[i].id);	//push() 方法可向数组的末尾添加一个或多个元素，并返回新的长度。
		}
		...
		$.messager.confirm("系统提示","您确认要删掉这<font color=red>" + selectedRows.length + "</font>条数据吗？",function(r){
			if(r){	//如果点击确定的话
				...
				$.post("gradeDelete",{delIds:ids},function(result){			//"gradeDelete",{delIds:ids}
																			//  请求地址		     请求参数
					//如果删除成功的话
					if(result.success){
...
<body  style="margin: 5px">
	...
	<table id="dg" title="班级信息" class="easyui-datagrid" fitColumns="true"
	 pagination="true" rownumbers="true" fit="true" url="gradeList" toolbar="#tb" iconCls="icon-edit"
	 singleSelect="true" idField="itemid">
		<thead>
			<tr>
				<th fild="cb" checkbox="true">
